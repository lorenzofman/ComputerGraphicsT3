/* 
	...
	C�digo formatado, m�todos convertidos para Pascal Case (para manter a formata��o nas classes mais abstratas),  traduzido para ingl�s e convertido para classe por: Lorenzo Schwertner Kaufmann (23/03/2020)
	M�todos que usavam o valor literal 6.27 ao inv�s de PI_2 (6.28) para n�o fechar todos pontos do c�rculo foram substitu�dos por loops inteiros que n�o causam confus�o. Itera n-1 vezes
	Parametros de DrawPolygon alterados para uma estrutura que cont�m x, y
	Original dispon�vel em http://www-usr.inf.ufsm.br/~pozzer/
*/
...
void (*Canvas2D::OnScreenDimensionUpdate)(void);
Canvas2D::Canvas2D(int* h, int* w, std::string title,
	...
	void(OnScreenDimensionUpdate(void)))
{
	int argc = 0;
	glutInit(&argc, NULL);
	...
	scrHeight = h;
	scrWidth = w;
	Canvas2D::OnKeyDownCallback = OnKeyDownCallback;
	Canvas2D::OnKeyUpCallback = OnKeyUpCallback;
	Canvas2D::OnMouseUpdateCallback = OnMouseUpdateCallback;
	Canvas2D::OnRenderCallback = OnRenderCallback;
	Canvas2D::OnScreenDimensionUpdate = OnScreenDimensionUpdate;
...
void Canvas2D::SetColor(Color color)
{
	glColor3d(color.r, color.g, color.b);
}

void Canvas2D::ClearScreen(Color color)
{
